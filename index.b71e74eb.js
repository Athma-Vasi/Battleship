...
/**
 *  Handles click event on battleship ship selection button by disabling the button and other ship selection buttons by removing their event listeners until after the ship is placed on the game board. Also adds event listeners to the game board cells to handle (battleship) mouseenter, mouseleave, and click events.
 *
 * @function
 * @param {HTMLButtonElement} this - The button that was clicked.
 * @param {MouseEvent} ev - The event object.
 ...
 */ const handlePlayerClickOnCompMisses = function(ev) {
    ...
    //computers turn
    setTimeout((0, _computersTurn.computersTurn), 1500);
};
 ...
 */ async function renderBattleMessageHelper({ towardsCombatant , firedStatus , shipTypeHit , shipNumber  }) {
    ...
    let shipName = "";
    if (towardsCombatant === "comp") // the shipTypeHit is not passed in when the firedStatus is 'miss'
    {
 ...
 */ function generateProbabilisticFiringCoord() {
    ...
    else // if there havent been any hits yet, generates a random guess
